            ...
            for stream, entries in messages:
                for message_id, message in entries:
                    if message_id != last_id:
                        ...
                        r.set("adjustment_service_last_redis", int(time.time()))
    ...
    async with aiohttp.ClientSession() as session:
        ...
        while True:
            with data_lock:
                ...
                index_snapshot = list(index_price)

            i = 0
            while i < len(trades_snapshot):
                with data_lock:
                ...
                if current_time - last_check_time >= 10:
                    r.set("adjustment_service_last_check", current_time)
                    last_check_time = current_time

                if (
                    trade.get("alive") == False
                    or trade.get("entryTriggered") == False
                    or trade.get("tpTriggered")
                    or trade.get("slTriggered")
                    or trade.get("entrySide") == "UNDEFINED"
                    or trade.get("isDummy") == True
                ):
                    await asyncio.sleep(0.5)
                    i += 1
                    continue

                index_map = {item["name"]: item for item in index_snapshot}
                price = index_map.get(trade["indexName"], {}).get("price")
                if price is None:
                    await asyncio.sleep(0.5)
                    i += 1
                    continue

                if entry_criteria_met(trade, price):
                    matching_price_entry = next(
                        (entry for entry in price_snapshot if entry["id"] == trade["id"]),
                        None,
                    )
                    ...
                    sorted_spreads = sorted(
                        spread_premiums, key=lambda x: abs(x["spreadPremium"])
                    )
                    closest_to_zero_spreads = sorted_spreads[:leg_count]

                    try:
                        orderOptionDetails = place_order_strike_details(
                            closest_to_zero_spreads, trade
                        )

                        # async network request
                        get_existing_positions = await get_order_position(session,trade["id"])
                        removed, added = get_order_differences_by_exchange_id(
                            orderOptionDetails, get_existing_positions
                        )
                        print(removed, added)

                        if removed or added:
                            await handle_order_actions(trade, removed, added,session)  # async
                            await get_entry_price(session,removed)  # left sync as per original
                            await get_entry_price(session,added)  # left sync as per original
                            if removed:
                                await retry_update_position_details(session,removed)
                            if added:
                                await add_order_position(session,added)

                        if (
                            price >= trade["pointOfAdjustmentUpperLimit"]
                            and trade["pointOfAdjustment"] != 0
                        ):
                            new_upper_limit = (
                            ...
                            trade["pointOfAdjustmentUpperLimit"] = new_upper_limit
                            trade['pointOfAdjustmentLowerLimit'] = new_lower_limit
                            await retry_update_trade_details(
                                session,
                                trade["id"],
                                {"pointOfAdjustmentUpperLimit": new_upper_limit,"pointOfAdjustmentLowerLimit":new_lower_limit},
                            )
                        if (
                            price <= trade["pointOfAdjustmentLowerLimit"]
                            and trade["pointOfAdjustment"] != 0
                        ):
                            new_upper_limit = (
                                trade["pointOfAdjustmentUpperLimit"]
...
# ----- Entry Criteria ----
def entry_criteria_met(trade, price):
    upper_limit = trade['pointOfAdjustmentUpperLimit']
    lower_limit = trade['pointOfAdjustmentLowerLimit']
    point_of_adjustment = trade['pointOfAdjustment']

    if (upper_limit != 0 or lower_limit != 0) and (price >= upper_limit or price <= lower_limit):
        return True
    if point_of_adjustment == 0:
        return True
    return False

# ---- Helper Functions ----
def get_order_differences_by_exchange_id(orderOptionDetails, existing_positions):
    existing_ids = {item['exchangeId'] for item in existing_positions}
    new_ids = {item['exchangeId'] for item in orderOptionDetails}

    removed_ids = existing_ids - new_ids
    added_ids = new_ids - existing_ids

    removed_dicts = [item for item in existing_positions if item['exchangeId'] in removed_ids]
    added_dicts = [item for item in orderOptionDetails if item['exchangeId'] in added_ids]

    return removed_dicts, added_dicts
...
async def get_order_position(session,trade_id, retries=3):
    for _ in range(retries):
        try:
            result = await make_get_request(
                session,
                url=f"http://localhost:3000/user/position?id={trade_id}",
            ...
            print(f"Retrying get_order_position: {e}")
            time.sleep(1)
    return {}
...
async def retry_update_position_details(session, position_list, retries=3):
    ...
    for _ in range(retries):
        try:
            ...
            print(f"[{time.strftime('%H:%M:%S')}] Retrying update_position_details: {e}")
...
async def add_order_position(session, orderOptionDetails, retries=3, semaphore=None):
    for _ in range(retries):
        try:
...
async def place_order_exit(session, trade, option):
    try:
...
async def place_order_entry(session, trade, option):
    try:
        res = await make_post_request(
            session=session,
            url="https://trading.bigul.co/interactive/orders",
...
async def handle_order_actions(trade, removed, added,session):

    tasks = []

    for position in removed:
        tasks.append(place_order_exit(session, trade, position))
    for option in added:
        tasks.append(place_order_entry(session, trade, option))

    results = await asyncio.gather(*tasks, return_exceptions=True)

    # Assign order IDs to positions/options
    for i, position in enumerate(removed):
        position['exitAppOrderId'] = results[i]
    for j, option in enumerate(added):
        option['entryAppOrderId'] = results[len(removed) + j]
        option['exitAppOrderId'] = 0

    print(f"[{time.strftime('%H:%M:%S')}] ✅ Simultaneous Entry & Exit Orders for {trade['indexName']} ({trade['id']})")
...
def on_option_premium(data):
    ...
    try:
        ...
        if current_time_now - socket_last_check_time  >= 10:
            r.set("adjustment_service_last_socket", int(time.time()))
